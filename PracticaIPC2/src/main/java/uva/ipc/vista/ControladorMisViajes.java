...
public class ControladorMisViajes {
    ...
    public void editarBillete() {
        if (vista.getSelectedBillete() == null) {
            Utiles.dialogoError("Por favor, seleccione usted un billete");
        } else if (LocalDateTime.of(vista.getSelectedBillete().getFecha(), vista.getSelectedBillete().getHora()).isBefore(LocalDateTime.now())) {
            Utiles.dialogoError("El billete está pasado");
        } else {
            Main.getModelo().setSelectedBilleteEdicion(vista.getSelectedBillete());
            vista.actualizarVistaEditarBillete(Main.getModelo().getViajesPaso2(), vista.getSelectedBillete());
            vista.activarVistaEditarBillete();
        }
    }

    /**
     * Muestra la vista de Billetes del usuario cuando se decide cancelar la
     * edicion de un billete
     */
